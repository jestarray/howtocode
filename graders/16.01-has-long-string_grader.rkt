...
(define ASSIGNMENT-NAME 'has-long-string)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure has-long-string? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "has-long-string?#1" "err" (has-long-string? empty) #false 1)
(@test "has-long-string?#2" "err" (has-long-string? (cons "butterfly" empty)) #true 1)
(@test "has-long-string?#3" "err" (has-long-string? (cons "meow" (cons "butterfly" empty))) #true 1)
