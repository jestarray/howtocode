...
(define ASSIGNMENT-NAME 'multi-bubble-points)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure bubble-all-points 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "bubble-all-points#0" "err" (bubble-all-points empty) empty 1)
(@test "bubble-all-points#1" "err" (bubble-all-points (list (make-point 30 50))) ((submission-eval) '(list (make-point 30 (- 50 1)))) 1)
(@test "bubble-all-points#2" "err" (bubble-all-points (list (make-point 30 50) (make-point 30 -1))) ((submission-eval) '(list (make-point 30 (- 50 1)))) 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
