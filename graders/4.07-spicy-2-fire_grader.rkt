...
(define ASSIGNMENT-NAME 'spicy->fire)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure spicy->fire 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 6)


(@test "1" "err" (spicy->fire "Not Spicy") "" 1)
(@test "2" "err" (spicy->fire "Mild") "🔥" 1)
(@test "3" "err" (spicy->fire "Medium") "🔥🔥" 1)
(@test "4" "err" (spicy->fire "Hot") "🔥🔥🔥" 1)
(@test "5" "err" (spicy->fire "Extra Hot") "🔥🔥🔥🔥" 1)
(@test "6" "err" (spicy->fire "Extremely Hot") "🔥🔥🔥🔥🔥" 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
