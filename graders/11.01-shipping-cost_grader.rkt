...
(define ASSIGNMENT-NAME 'shipping-cost)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure shipping-cost 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "shipping-cost#1" "err" (shipping-cost (make-standard 23)) (* 23 1) 1)
(@test "shipping-cost#2" "err" (shipping-cost (make-express 33 21)) (+ 33 (* 21 2)) 1)
(@test "shipping-cost#3" "err" (shipping-cost (make-international 40 33)) (+ 40 (* 33 5)) 1)
