...
(define ASSIGNMENT-NAME 'word-length)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure word-length 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "word-length#1"  "err" (word-length empty) empty 1)
(@test "word-length#2"  "err" (word-length (cons "woof" empty)) (cons "woof-4" empty) 1)
(@test "word-length#3"  "err" (word-length (cons "hello" (cons "meow" empty))) (cons "hello-5" (cons "meow-4" empty)) 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
