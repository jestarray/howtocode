...
(define ASSIGNMENT-NAME 'only-evens)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure only-evens 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 4)

(@test "only-evens#1" "err" (only-evens empty) empty 1)
(@test "only-evens#2" "err" (only-evens (cons 1 empty)) empty 1)
(@test "only-evens#3" "err" (only-evens (cons 1 (cons 2 (cons 3 empty)))) (cons 2 empty) 1)
(@test "only-evens#4" "err" (only-evens (cons 1 (cons 2 (cons 3 (cons 4 empty))))) (cons 2 (cons 4 empty)) 1)
