...
(define LIMIT 20) ; a size measure in terms of pixels 
 
(define small-triangle (triangle LIMIT "outline" "red"))
...
(check-expect (s-triangle LIMIT) small-triangle)
(check-expect (s-triangle (* 2 LIMIT))
              (above small-triangle
                     (beside small-triangle small-triangle)))

(define (s-triangle side)
  (cond
    [(<= side LIMIT) ; stop when we hit the smallest limit
     (triangle side "outline" "red")]
    [else
     (local [(define half-sized (s-triangle (/ side 2)))]
       (above half-sized
