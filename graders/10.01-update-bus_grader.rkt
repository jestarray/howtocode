...
(define ASSIGNMENT-NAME 'update-bus)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure update-bus 2)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 2)

(@test "update-bus#1" "err" (update-bus (make-bus 1 (make-driver "Bob" 1)) (make-driver "Alice" 1)) ((submission-eval) '(make-bus 1 (make-driver "Alice" 1))) 1)
(@test "update-bus#2" "err" (update-bus (make-bus 1 (make-driver "Bob" 1)) (make-driver "jack" 2)) ((submission-eval) '(make-bus 1 (make-driver "Bob" 1))) 1)
