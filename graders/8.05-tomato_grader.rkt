...
(@test "tock" "err: TRY SPEED: 4, ROTATE-SPEED -4" (tock t0) ((submission-eval) '(make-tomato (+ 4 0) (+ -4 0))) 1)

(@test "handle-key#1" "did not reset" (handle-key (make-tomato 30 90) "r") ((submission-eval) '(make-tomato 0 0)) 1)
(@test "handle-key#1" "did not stay the same" (handle-key (make-tomato 42 12) " ") ((submission-eval) '(make-tomato 42 12)) 1)
