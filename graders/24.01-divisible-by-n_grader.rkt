...
(define ASSIGNMENT-NAME 'divisible-by-n)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure divisible-by-n 2)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "divisible-by-n#1" "err" (divisible-by-n (list 1 2 3 4) 2) (list 2 4) 1)
(@test "divisible-by-n#2" "err" (divisible-by-n (list 1 2 3 4) 3) (list 3) 1)
(@test "divisible-by-n#3" "err" (divisible-by-n (list 1 3 5) 2) empty 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
