...
(define ASSIGNMENT-NAME 'shape-area)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure shape-area 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "shape-area#1" "err" (shape-area (make-rect 30 20)) (* 30 20) 1)
(@test "shape-area#2" "err" (shape-area (make-square 10))  (* 10 10) 1)
; skip circle cause pi?
