...
(define ASSIGNMENT-NAME 'all-evens)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure all-evens? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "all-evens?#1" "err" (all-evens? empty) #true 1)
(@test "all-evens?#2" "err" (all-evens? (cons 2 (cons 3 empty))) #false 1)
(@test "all-evens?#3" "err" (all-evens? (cons 4 (cons 6 empty))) #true 1)
