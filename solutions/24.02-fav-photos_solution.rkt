...
(: to-frame (String [ListOf PhotoSig] -> [ListOf PhotoSig]))
; produces a list of photos that are favourites and belong to the given album
(check-expect (to-frame "fam" (list cat wall selfie)) empty)
(check-expect (to-frame "me" (list cat wall selfie)) (list selfie))
(check-expect (to-frame "pets" (list cat wall dog)) (list cat))
(define (to-frame find-album lst)
  (local
    [(define (fav-in-album? ph)
       (and (string=? (photo-album ph) find-album) (photo-favorite ph)))]
    (filter fav-in-album? lst)))