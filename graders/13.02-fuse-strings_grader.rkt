...
(define ASSIGNMENT-NAME 'fuse-strings)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure fuse-strings 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)


(@test "fuse-strings#1" "err" (fuse-strings empty) "" 1)
(@test "fuse-strings#2" "err" (fuse-strings (cons "meow" empty)) "meow" 1)
(@test "fuse-strings#3" "err" (fuse-strings (cons "doo" (cons "moof" empty))) "doomoof" 1)
