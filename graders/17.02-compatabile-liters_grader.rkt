...
(define ASSIGNMENT-NAME 'compatabile-liters)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure compatabile-liters 2)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 8)

(@test "compatabile-liters#1" "err" (compatabile-liters empty "AB+") 0 1)
(@test "compatabile-liters#2" "err" (compatabile-liters (cons "A-" (cons "O-" empty)) "AB-") 2 1)
(@test "compatabile-liters#3" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "AB+") 8 1)
(@test "compatabile-liters#4" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "AB-") 4 1)
(@test "compatabile-liters#5" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "A+") 4 1)
(@test "compatabile-liters#6" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "B+") 4 1)
(@test "compatabile-liters#7" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "A-") 2 1)
(@test "compatabile-liters#8" "err" (compatabile-liters (cons "AB+" (cons "AB-" (cons "A+"  (cons "A-"  (cons "B+"  (cons "B-"  (cons "O+" (cons "O-" empty)))))))) "B-") 2 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
