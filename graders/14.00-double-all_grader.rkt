...
(define ASSIGNMENT-NAME 'double-all)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure double-all 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)


(@test "double-all#1" "err" (double-all empty) empty 1)
(@test "double-all#2" "err" (double-all (cons 2 empty)) (cons 4 empty) 1)
(@test "double-all#3" "err" (double-all (cons 3 (cons 5 (cons 2 empty)))) (cons 6 (cons 10 (cons 4 empty))) 1)
