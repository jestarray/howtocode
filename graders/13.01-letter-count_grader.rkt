...
(define ASSIGNMENT-NAME 'letter-count)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure letter-count 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)


(@test "letter-count#1" "err" (letter-count empty) 0 1)
(@test "letter-count#2" "err" (letter-count (cons "meow" empty)) 4 1)
(@test "letter-count#3" "err" (letter-count (cons "doo" (cons "moof" empty))) 7 1)
