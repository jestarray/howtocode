...
; failing-letter? : (LetterGrade -> Boolean)
; produces #true if the given grade is D or lower
(check-expect (failing-letter? A) #false)
(check-expect (failing-letter? B) #false)
(check-expect (failing-letter? C) #false)
(check-expect (failing-letter? D) #true)
(check-expect (failing-letter? F) #true)

; (define (failing-letter? lg) #false)
(define (failing-letter? lg)
  (cond
    [(string=? lg A) #false]
    [(string=? lg B) #false]
    [(string=? lg C) #false]
