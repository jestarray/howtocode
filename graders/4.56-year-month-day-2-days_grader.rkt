...
(define ASSIGNMENT-NAME 'year-month-day->days)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure year-month-day->days 3)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 4)
(@test "1" "err" (year-month-day->days 0000 "January" 1) 0 1)
(@test "2" "err" (year-month-day->days 0001 "January" 1) 365 1)
(@test "3" "err" (year-month-day->days 0001 "February" 1) (+ 365 31) 1)
(@test "4" "err" (year-month-day->days 2017 "August" 28) 736444 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
