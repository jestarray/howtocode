...
(define ASSIGNMENT-NAME 'failing-letter?)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure failing-letter? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 5)

(@test "1" "err" (failing-letter? "A") #false 1)
(@test "2" "err" (failing-letter? "B") #false 1)
(@test "3" "err" (failing-letter? "C") #false 1)
(@test "4" "err" (failing-letter? "D") #true 1)
(@test "5" "err" (failing-letter? "F") #true 1)
