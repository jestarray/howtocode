...
(define ASSIGNMENT-NAME 'update-battery)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure update-battery 2)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "update-battery#1" "err" (update-battery tesla (make-battery 1 100)) ((submission-eval) '(make-evcar "Tesla" (make-battery 1 100))) 1)
(@test "update-battery#2" "err" (update-battery byd (make-battery 2 50)) ((submission-eval) '(make-evcar "BYD" (make-battery 2 100))) 1)
(@test "update-battery#3" "err" (update-battery byd (make-battery 1 100)) ((submission-eval) '(make-evcar "BYD" (make-battery 2 100))) 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
