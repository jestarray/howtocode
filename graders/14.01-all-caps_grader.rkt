...
(define ASSIGNMENT-NAME 'all-caps)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure all-caps 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)


(@test "all-caps#1" "err" (all-caps empty) empty 1)
(@test "all-caps#2" "err" (all-caps (cons "happy" empty)) (cons "HAPPY" empty) 1)
(@test "all-caps#3" "err" (all-caps (cons "moo" (cons "cow" (cons "v" empty)))) (cons "MOO" (cons "COW" (cons "V" empty))) 1)
