...
(define ASSIGNMENT-NAME 'broadcast-status)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure broadcast-status 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "1" "err" (broadcast-status 100) "Altitude is 100 kms." 1)
(@test "2" "err" (broadcast-status 1) "Altitude is 1 kms." 1)
(@test "3" "err" (broadcast-status #false) "The rocket has landed!" 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
