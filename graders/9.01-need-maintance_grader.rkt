...
(define ASSIGNMENT-NAME 'need-maintance)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure need-maintance? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 6)

(@test "need-maintance?#1" "err" (need-maintance? (make-vehicle "Honda" (make-engine 2000 10 #false))) ((submission-eval) #true) 1)
(@test "need-maintance?#2" "err" (need-maintance? (make-vehicle "Toyota" (make-engine 2000 10 #true))) ((submission-eval) #true) 1)
(@test "need-maintance?#3" "err" (need-maintance? (make-vehicle "BYD" (make-engine 2000 5 #true))) ((submission-eval) #true) 1)
(@test "need-maintance?#4" "err" (need-maintance? (make-vehicle "Volvo" (make-engine 2000 5 #false))) ((submission-eval) #true) 1)
(@test "need-maintance?#5" "err" (need-maintance? (make-vehicle "Xpeng" (make-engine 2000 40 #false))) ((submission-eval) #false) 1)
(@test "need-maintance?#6" "err" (need-maintance? (make-vehicle "NIO" (make-engine 2000 40 #true))) ((submission-eval) #true) 1)
