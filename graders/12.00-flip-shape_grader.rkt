...
(define ASSIGNMENT-NAME 'flip-shape)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure flip-shape 1)
        (!procedure grow-shape 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 4)

(@test "flip-shape#1" "err" (flip-shape (make-disc 30)) ((submission-eval) '(make-box 30 30)) 1)
(@test "flip-shape#2" "err" (flip-shape (make-box 30 30)) ((submission-eval) '(make-disc 30)) 1)

(@test "grow-shape#1" "err" (grow-shape (make-disc 30)) ((submission-eval) '(make-disc 31)) 1)
(@test "grow-shape#1" "err" (grow-shape (make-box 20 30)) ((submission-eval) '(make-box 21 31)) 1)
        )
(post:
 (add-score-to-report!)
 (write-report)
