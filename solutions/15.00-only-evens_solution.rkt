...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 15.00-only-evens_solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (only-evens (cons 1 (cons 2 (cons 3 empty))))
              (cons 2 empty))
(check-expect (only-evens (cons 1 (cons 2 (cons 3 (cons 4 empty)))))
              (cons 2 (cons 4 empty)))
;(define (only-evens num-lst) empty)
(define (only-evens num-lst)
  (cond
    [(empty? num-lst) empty]
    [else
     (if (even? (first num-lst))
         (cons (first num-lst) (only-evens (rest num-lst)))
         (only-evens (rest num-lst)))]))