...
(define ASSIGNMENT-NAME 'sum-n-odds)
...
(check: :language 'lang/htdp-intermediate-lambda
        ...
        (!procedure sum-n-odds 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "sum-n-odds#1" "err" (sum-n-odds 0) 0 1)
(@test "sum-n-odds#2" "err" (sum-n-odds 1) (+ 0 1) 1)
(@test "sum-n-odds#3" "err" (sum-n-odds 3) (+ 0 1 3 5) 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
