...
(define ASSIGNMENT-NAME 'express-lane)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure can-express-lane? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 5)

(@test "express#1" "err" (can-express-lane? (make-gas-car 1)) #false 1)
(@test "express#2" "err" (can-express-lane? (make-gas-car 2)) #true 1)
(@test "express#3" "err" (can-express-lane? (make-gas-car 3)) #true 1)
(@test "express#4" "err" (can-express-lane? (make-truck 40)) #false 1)
(@test "express#5" "err" (can-express-lane? (make-motorcycle)) #true 1)
