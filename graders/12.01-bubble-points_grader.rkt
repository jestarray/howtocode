...
(define ASSIGNMENT-NAME 'bubble-points)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure move-up 1)
        (!procedure bubble-points 1)
        (!procedure add-point 3)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 6)
(@test "move-up#1" "err" (move-up (make-point 5 250)) ((submission-eval) '(make-point 5 249)) 1)

(@test "bubble-points#1" "err" (bubble-points (make-none)) ((submission-eval) '(make-none)) 1)
(@test "bubble-points#2" "err" (bubble-points (make-single (make-point 5 250))) ((submission-eval) '(make-single (make-point 5 249))) 1)
(@test "bubble-points#3" "err" (bubble-points (make-couple (make-point 5 250) (make-point 10 330))) ((submission-eval) '(make-couple (make-point 5 249) (make-point 10 329))) 1)

(@test "add-point#1" "err" (add-point (make-none) 20 30) ((submission-eval) '(make-single (make-point 20 30))) 1)
(@test "add-point#2" "err" (add-point (make-single (make-point 20 30)) 50 60) ((submission-eval) '(make-couple (make-point 50 60) (make-point 20 30))) 1)
        )
(post:
 (add-score-to-report!)
 (write-report)
