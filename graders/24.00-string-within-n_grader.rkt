...
(define ASSIGNMENT-NAME 'string-within-n)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure string-within-n 2)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 3)

(@test "string-within-n#1" "err" (string-within-n empty 0) empty 1)
(@test "string-within-n#2" "err" (string-within-n (list "bob" "smith" "lee") 4) (list "bob" "lee") 1)
(@test "string-within-n#2" "err" (string-within-n (list "bob" "smith" "lee") 1) empty 1)
; (@test "collapse#4" "err" (collapse string-append "" (list "a" "z")) "az" 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
