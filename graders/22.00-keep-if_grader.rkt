...
(define ASSIGNMENT-NAME 'keep-if)
...
(@test "keep-if#1" "err" (keep-if positive? empty) empty 1)
(@test "keep-if#2" "err" (keep-if positive? (list 9 3 -2 -4)) (list 9 3) 1)
(@test "keep-if#3" "err" (keep-if negative? (list 9 3 -2 -4)) (list -2 -4) 1)
;(@test "keep-if#2" "err" (keep-if boolean? (list "a" #false "b")) (list #false) 1)
;(@test "keep-if#3" "err" (keep-if number? (list #true 2 4)) (list 2 4) 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
