...
(define ASSIGNMENT-NAME 'next-month)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure next-month 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 12)

(@test "1" "err" (next-month "January") "February" 1)
(@test "2" "err" (next-month "February") "March" 1)
(@test "3" "err" (next-month "March") "April" 1)
(@test "4" "err" (next-month "April") "May" 1)
(@test "5" "err" (next-month "May") "June" 1)
(@test "6" "err" (next-month "June") "July" 1)
(@test "7" "err" (next-month "July") "August" 1)
(@test "8" "err" (next-month "August") "September" 1)
(@test "9" "err" (next-month "September") "October" 1)
(@test "10" "err" (next-month "October") "November" 1)
(@test "11" "err" (next-month "November") "December" 1)
(@test "12" "err" (next-month "December") "January" 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
