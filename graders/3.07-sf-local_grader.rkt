...
(define ASSIGNMENT-NAME 'sf-local)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure sf-local? 1)

        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 6)

(@test "1" "failed" (sf-local? "4159287278") #true 1)
(@test "2" "failed" (sf-local? "5109287278") #true 1)
(@test "3" "failed" (sf-local? "6509287278") #true 1)
(@test "4" "failed" (sf-local? "1509287278") #false 1)
(@test "5" "failed" (sf-local? "0509287278") #false 1)
(@test "6" "failed" (sf-local? "050") #false 1)
        ; (println submission)
        )
(post:
 (add-score-to-report!)
