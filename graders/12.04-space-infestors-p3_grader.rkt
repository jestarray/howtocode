...
(define ASSIGNMENT-NAME 'space-infestors-p3)
...
(check: :language 'lang/htdp-intermediate
        ...
        (!procedure bullet-hit-enemy? 2)
        (!procedure can-fire-bullet? 1)
        (!procedure handle-key 2)
        (!procedure check-win-lose? 1)
        (!procedure update-game 1)
        ; MAX SCORE SHOULD BE (N / TOTAL-UNIT-TESTS)
        (set-test-max-score! 6)

(@test "can-fire-bullet#1" "err" (can-fire-bullet? #false) #true 1)
(@test "can-fire-bullet#2" "err" (can-fire-bullet? (make-point 20 -1)) #true 1)
(@test "can-fire-bullet#3" "err" (can-fire-bullet? (make-point 20 20)) #false 1)

(@test "check-win-lose#1" "err" (check-win-lose? (make-game (make-tank 5 0) #false #false 1)) #false 1)
(@test "check-win-lose#2" "err" (check-win-lose? (make-game (make-tank 5 0) #false #false 3)) #true 1)
(@test "check-win-lose#3" "err" (check-win-lose? (make-game (make-tank 5 0) #false (make-point 50 999) 0)) #true 1)

        ; not testing because draw image functions are subjective?
        )
(post:
 (add-score-to-report!)
 (write-report)
